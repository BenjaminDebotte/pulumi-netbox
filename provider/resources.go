...
import (
	...
	"github.com/BenjaminDebotte/pulumi-netbox/provider/pkg/version"
)
...
func convertName(tfname string) (module string, name string) {
	...
	contract.Assertf(tfNameItems[0] == "netbox", "Invalid snake case name %s. Does not start with netbox", tfname)
...
func makeDataSource(ds string) tokens.ModuleMember {
	...
	return tfbridge.MakeDataSource("netbox", mod, "get"+name)
}

func makeResource(res string) tokens.Type {
	mod, name := convertName(res)
	return tfbridge.MakeResource("netbox", mod, name)
}
...
func Provider() tfbridge.ProviderInfo {
	...
	prov := tfbridge.ProviderInfo{
		...
		TFProviderVersion: "43c2dc48a2a3fee3ff3eed4a3d89db16c6dbe0b6",
		...
		Golang: &tfbridge.GolangInfo{
			ImportBasePath: filepath.Join(
				fmt.Sprintf("github.com/BenjaminDebotte/pulumi-%[1]s/sdk/", "netbox"),
